...
static tim_ConfigStruct_t const         tim_PeriphConf[ TIM_PERIPH_CNT ] =
{
  ...
  { .PeriphReg = TIM1, .PeriphRcc = RCC_PERIPH_TIM1, .Resolution = TIM_RESOLUTION_16BIT, .ChannelCount = 6u,
    .IsrConfig = { .GeneralIsr        = Tim_Tim1_General_IsrHandler              , .GeneralIrqId        = NVIC_PERIPH_IRQ_SIZE,
                   .BreakIsr          = Tim_Tim1_Break_Tim15_General_IsrHandler  , .BreakIrqId          = NVIC_PERIPH_IRQ_TIM1_BRK_TIM15,
                   .UpdateIsr         = Tim_Tim1_Update_Tim16_General_IsrHandler , .UpdateIrqId         = NVIC_PERIPH_IRQ_TIM1_UP_TIM16,
                   .TriggerCommIsr    = Tim_Tim1_Trigger_Tim17_General_IsrHandler, .TriggerCommIrqId    = NVIC_PERIPH_IRQ_TIM1_TRG_COM_TIM17,
  ...
  { .PeriphReg = TIM7 , .PeriphRcc = RCC_PERIPH_TIM7 , .Resolution = TIM_RESOLUTION_16BIT, .ChannelCount = 1u,
    .IsrConfig = { .GeneralIsr        = Tim_Tim7_General_IsrHandler              , .GeneralIrqId        = NVIC_PERIPH_IRQ_TIM7_DAC,
  ...
  { .PeriphReg = TIM15, .PeriphRcc = RCC_PERIPH_TIM15, .Resolution = TIM_RESOLUTION_16BIT, .ChannelCount = 6u,
    .IsrConfig = { .GeneralIsr        = Tim_Tim1_Break_Tim15_General_IsrHandler  , .GeneralIrqId        = NVIC_PERIPH_IRQ_TIM1_BRK_TIM15,
  ...
  { .PeriphReg = TIM16, .PeriphRcc = RCC_PERIPH_TIM16, .Resolution = TIM_RESOLUTION_16BIT, .ChannelCount = 6u,
    .IsrConfig = { .GeneralIsr        = Tim_Tim1_Update_Tim16_General_IsrHandler , .GeneralIrqId        = NVIC_PERIPH_IRQ_TIM1_UP_TIM16,
  ...
  { .PeriphReg = TIM17, .PeriphRcc = RCC_PERIPH_TIM17, .Resolution = TIM_RESOLUTION_16BIT, .ChannelCount = 6u,
    .IsrConfig = { .GeneralIsr        = Tim_Tim1_Trigger_Tim17_General_IsrHandler, .GeneralIrqId        = NVIC_PERIPH_IRQ_TIM1_TRG_COM_TIM17,
...
tim_RequestState_t Tim_Init( tim_PeriphConfig_t * const timConfig )
{
    ...
    if( TIM_NULL_PTR != timConfig )
    {
        ...
        /* Configuration can be processed only if peripheral is inactive */
        Tim_Set_PeriphInactive( timConfig->PeriphId );

        /* Initialize timer base functionality */
        Tim_InitBase( timConfig );
        ...
        Tim_Set_PeriphActive( timConfig->PeriphId );
    }
    else
    {
        retState = TIM_REQUEST_ERROR;
